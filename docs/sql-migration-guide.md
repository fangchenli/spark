...
- Since Spark 4.4, a timezone-naive pandas timestamp column backed by `pd.ArrowDtype` (for example `timestamp[us][pyarrow]`) is interpreted in the session timezone when converted to Spark, consistent with numpy-backed `datetime64` columns and with the documented behavior of `spark.sql.session.timeZone`. In 4.3 and earlier such columns were interpreted as UTC, so in a non-UTC session `createDataFrame` from pandas and the results of pandas UDFs returning these columns could be offset by the session's UTC offset. To keep the previous interpretation, make the column timezone-aware in UTC (for example `timestamp[us, tz=UTC][pyarrow]`), which is passed through unchanged.
